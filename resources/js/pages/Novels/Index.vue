...
<template>
    ...
    <div class="flex h-full flex-1 flex-col gap-6 p-6">
        ...
        <!-- Novel Grid -->
        <div v-if="novels.length > 0" class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            <div
        ...
        <div v-else class="flex flex-col items-center justify-center rounded-xl border border-dashed border-gray-300 p-12 text-center dark:border-gray-800">
            ...
            <button
                @click="showModal = true"
                class="mt-4 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow hover:bg-indigo-500"
            >
                Add Novel
            </button>
        </div>
